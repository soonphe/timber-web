<template>
  <div v-clickoutside="handleClose" class="el-select el-popper">
    <div
      v-if="multiple"
      ref="tags"
    ...
    <transition name="el-zoom-in-top" @after-leave="doDestroy" @after-enter="handleMenuEnter">
      <el-select-menu v-show="visible && emptyText !== false" ref="popper">
        <el-scrollbar
...
export default {
  ...
  computed: {
    iconClass() {
      const criteria = this.clearable && !this.disabled &&
  ...
  methods: {
    ...
    getOverflows() {
      if (this.dropdownUl && this.selected && this.selected.$el) {
        const selectedRect = this.selected.$el.getBoundingClientRect()
        const popperRect = this.$refs.popper.$el.getBoundingClientRect()
    ...
    resetScrollTop() {
      const bottomOverflowDistance = this.options[this.hoverIndex].$el.getBoundingClientRect().bottom -
        this.$refs.popper.$el.getBoundingClientRect().bottom
      const topOverflowDistance = this.options[this.hoverIndex].$el.getBoundingClientRect().top -
